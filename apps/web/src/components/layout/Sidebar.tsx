...
interface SidebarProps {
  ...
  onToggle: () => void
  onNavigate?: () => void
  /** Sheet内で表示される場合true（ヘッダー非表示） */
  inSheet?: boolean
}

export function Sidebar({ collapsed, onToggle, onNavigate, inSheet = false }: SidebarProps) {
  ...
  return (
    <div className={`${inSheet ? 'w-full h-full' : 'w-60 shrink-0 border-r h-screen sticky top-0'} bg-white flex flex-col`}>
      {/* Header — Sheet内では非表示 */}
      {!inSheet && (
      ...
      {pendingCount > 0 && (
        <button
          ...
          className="mx-3 mb-1 flex items-center gap-2 px-2 py-2 rounded-md text-sm hover:bg-neutral-50 text-muted-foreground min-h-[44px]"
        >
