...
const pageAgents = new Map<string, { agentId: string; trustScore: number } | null>()
...
export const agentBridgeExtension: Extension = {
  async onLoadDocument({ documentName }) {
    ...
    const { data } = await supabase
      .from('pages')
      .select('agent_id, agents(trust_score)')
      .eq('id', documentName)
      .single()

    if (data?.agent_id) {
      const agent = data.agents as unknown as { trust_score: number } | null
      pageAgents.set(documentName, {
...
export async function refreshPageAgent(pageId: string) {
  ...
  if (data?.agent_id) {
    const agent = data.agents as unknown as { trust_score: number } | null
    const info = { agentId: data.agent_id, trustScore: agent?.trust_score ?? 0 }
